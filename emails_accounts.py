"""
Fichier de configuration des adresses emails.
Vous pouvez modifier ces variables pour changer les adresses et les noms d'expédition.
Note : Les adresses d'expédition (SENDER_EMAIL) doivent être validées sur votre compte Brevo.
"""

MAIRIE_CONTACT_RECEPTION_EMAIL = "mairie@dhuizon.fr"

# acuses de reception
MAIRIE_SENDER_EMAIL = "mairie@dhuizon.fr"
MAIRIE_SENDER_NAME = "Mairie de Dhuizon"

# refuser ou accepter au centre de loisirs
CENTRE_LOISIRS_SENDER_EMAIL = "centre-de-loisirs@dhuizon.fr"
CENTRE_LOISIRS_SENDER_NAME = "Centre de Loisirs de Dhuizon"

# inscription garderie et cantine
PERISCOLAIRE_RECEPTION_EMAIL = "mairie@dhuizon.fr"
