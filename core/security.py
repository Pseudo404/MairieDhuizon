...
def get_client_ip(request):
    if getattr(settings, 'TRUST_X_FORWARDED_FOR', False):
        # getattr "TRUST_X_FORWARDED_FOR" si pas trouver False (pas d'erreur)
        x_forwarded_for = request.META.get('HTTP_X_FORWARDED_FOR')
        if x_forwarded_for:
            #si proxy il y a plusieurs IP, on prend la première celle du client d'origine
...
def is_admin_access_allowed(request):
    ...
    #si la securité est activer (prod oui)
    ...
    #en dev toujours accès via local
